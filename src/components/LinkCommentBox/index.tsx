...
const LinkCommentBox = ({
              ...
              (res) => {
                if (res) {
                  if (res.error) {
                    ...
                    chrome.runtime.sendMessage(
                      { action: BJActions.REMOVE_DATA_FOR_LINK },
                      (res) => {
                        if (res.error) {
                          console.log(res.error)
                        } else {
                          ...
                          chrome.tabs.query(
                            ...
                            (tabs) => {
                              ...
                              chrome.runtime.sendMessage(
                                { action: BJActions.SHOW_CIRCLE_COUNT, url },
                                (response) => {
  ...
  return (
    <div className="w-[333px] rounded-2.5xl bg-branding pb-5">
      ...
      {statusMessage && (
        <p className="text-brand text-xs leading-5 font-medium px-3">{statusMessage}</p>
      )}
      {errorMessage && (
        ...
        <div className="w-full pt-4 px-5 flex justify-between items-center">
          <div className="flex items-center gap-x-2">
            <p className="text-base font-semibold leading-normal text-brand">
              {!isCheckingIfSentComment ? 'Share to' : 'Shared'}
            </p>
            <img
