...
import MyCircles from 'pages/Circles/CircleList/MyCircles'
import PageCircleList from 'pages/Circles/CircleList/PageCirclesList'
...
const CircleList = () => {
  ...
  const handleScroll = useCallback(() => {
    if (resultTextRef.current && resultTextRef.current?.getClientRects().item(0)?.top) {
      ...
      resultTextRef.current?.children
        ?.item(1)
        ?.classList.remove('font-medium', 'capitalize', 'text-lg', 'justify-between')
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col items-center gap-5 px-5">
        ...
        <ShareThoughtBox />
        <div
          className="w-full flex items-center justify-center py-3 gap-2 sticky top-0 z-40 bg-white"
          ref={resultTextRef}
        >
