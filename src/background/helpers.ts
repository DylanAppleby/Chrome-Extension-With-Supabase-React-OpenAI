...
const circleGenerationSuccessHandler = (
  ...
  getFromStorage(tabId?.toString()).then(async (generationStatus = {}) => {
    if (
      generationStatus &&
      ...
      const autoGeneratedStatus = (await getFromStorage(tabId.toString())).auto
      const currentUuid = autoGeneratedStatus.uuid
      ...
      if (
        ...
        getGeneratedCircles(pageUrl, limitedWords).then((reSubmittedResult: any) => {
          if (reSubmittedResult.length > 0) {
            circleGenerationSuccessHandler('auto', tabId, reSubmittedResult, uuid)
          } else {
        ...
        if (circleGenerationResult.length > 0) {
          circleGenerationSuccessHandler('auto', tabId, circleGenerationResult, uuid)
        } else {
          circleGenerationFailedHandler('auto', tabId)
        }
      }
